...
def fix_pot_header(pot_path: Path):
    ...
    try:
        ...
        print(f"  - 检测到非 UTF-8 编码，尝试以 cp936 回退读取...")
...
def run_pygettext(pygettext_path: Path, domain: str, sources: List[str], output_dir: Path):
    ...
    command = [
        'python', str(pygettext_path), '--no-location',
        '-d', domain, '-o', str(output_pot_file)
    ] + sources
...
async def main():
    """主函数，负责整个自动化流程"""
    pygettext_path_str = os.getenv("pygettext_path") or input(
        "请输入 'pygettext.py' 的绝对路径:\n")
